...
class DbNewsHandler():
    def __init__(self):
        ...
        self.INSERT = "INSERT INTO news(title, body, d) VALUES ('{}','{}','{}')"
        self.DELETE = "DELETE FROM news WHERE id={}"
    ...
    def add_news(self, title, body):
        ...
        yield self._start_cursor()
        yield self.cur.execute(self.INSERT.format(title,body,d))
        yield self.cur.execute(self.SELECT_ALL)
        news = [News(*e) for e in self.cur]
        yield self._close_cursor()
        return news 
    ...
    def delete_news(self, id):
        yield self._start_cursor()
        yield self.cur.execute(self.DELETE.format(id))
        yield self.cur.execute(self.SELECT_ALL)
        news = [News(*e) for e in self.cur]
        yield self._close_cursor()
        return news 